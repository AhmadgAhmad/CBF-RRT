...
class MA_CBF_RRT:
    ...
    def QP_controller(self,x_current,u_ref):
        ...
        x1 = x_current[:self.num_agents]
        x2 = x_current[self.num_agents:]

        self.m.remove(self.m.getConstrs())
        u1_ref = u_ref[:,0]
        u2_ref = u_ref[:,1]
        # Control Acutuator Constraints
        self.u1 = self.m.addVars(self.num_agents, 1, lb=self.u1_lower_lim, ub=self.u1_upper_lim,
        vtype=GRB.CONTINUOUS, name="velocity_constraints_x1")
        self.u2 = self.m.addVars(self.num_agents, 1, lb=self.u2_lower_lim, ub=self.u2_upper_lim,
        vtype=GRB.CONTINUOUS, name="velocity_constraints_x2")

        # Initialize Cost Function
        self.cost_func = np.sum((self.u1.select()-u1_ref)*(self.u1.select()-u1_ref)+(self.u2.select()-u2_ref)*(self.u2.select()-u2_ref))
        self.m.setObjective(self.cost_func, GRB.MINIMIZE)

        # CBF function
        for i in range(0,len(self.x_obstacle)):
            for j in range(self.num_agents):
                h = (x1[j]-self.x_obstacle[i][0])**2+(x2[j]-self.x_obstacle[i][1])**2-self.x_obstacle[i][2]**2
                
                u1_var = self.u1.select(j)
                u2_var = self.u2.select(j)
                lgh = 2*(x1[j]-self.x_obstacle[i][0])*u1_var[0] + 2*(x2[j]-self.x_obstacle[i][1])*u2_var[0]

                self.m.addConstr((lgh+self.k_cbf*h**self.p_cbf)>=0)

        # Add constraints to all agents w.r.t other agents
        for i in range(0,self.num_agents):
            for j in range(i+1,self.num_agents):
                # NEED TO DEFINE AGENT RADIUS somewhere else probably
                agentSize = .25
                h = (x1[i]-x1[j])**2 + (x2[i]-x2[j])**2 - 2*agentSize**2

                u1_i = self.u1.select(i)
                u2_i = self.u2.select(i)
                u1_j = self.u1.select(j)
                u2_j = self.u2.select(j)
                lgh = 2*(x1[i]-x1[j])*(u1_i[0]-u1_j[0]) + 2*(x2[i]-x2[j])*(u2_i[0]-u2_j[0])
        ...
        u = np.array([self.solution[i].x for i in range(len(self.solution))])
        '''
        ...
        '''
        return np.expand_dims(u, axis=1)
