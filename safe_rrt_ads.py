...
import csv
import time
from simulation_closedform_ads import MA_CBF_RRT
...
class RRT():
    ...
    def __init__(self, start, goal, obstacleList, expandDis=0.5, goalSampleRate=5,u_ref_nominal=2.0):
        """
        Setting Parameter
        ...
        while not all(newNode.atGoal):
            ...
            for i in range(self.num_agents):
                if startNode.atGoal[i]:
                    ...
                    sampled_theta[i] = random.gauss(desired_theta, 0.3)
    ...
    def DrawGraph(self, path_x=[], path_y=[], axislim=[0, 10, 0, 10]):  # pragma: no cover
        """
        Draw Graph
        """
        ...
        if len(path_x) > 0:
            for i in range(self.num_agents):
                if i == 0:
                    plt.plot(path_x[i], path_y[i], 'b-', ms=18,label='Path')
                else:
                    plt.plot(path_x[i], path_y[i], 'b-', ms=18)
...
class World():
    ...
    def circle_obstacles(self, num_obstacles, circle_radius=4, obst_radius=0.25):
        ...
        for i in range(num_obstacles):
            ...
            radius = random.random() * (circle_radius - 1)
            x = math.cos(angle) * radius + circle_radius+1
            y = math.sin(angle) * radius + circle_radius+1
            self.obstacleList.append([x,y,obst_radius])
...
def main():
    ...
    ## Simple: 2 agents with 2 obstacles
    # obstacleList = [[3.0,5.0,0.5], [3.0,1.0,.5]]
    # init_pos = np.array([[1.0,1.0],[5.0,5.0]])
    # final_goal = np.array([[5.0,1.0],[1.0,5.0]])

    ## Simple 2 agents passing
    # obstacleList = []
    #obstacleList = [[4.0,2.0,0.5],[2.0,4.0,0.5]]
    #nit_pos = np.array([[1.0,1.0],[5.0,5.0], [1.0,5.0], [0.0,3.0], [3.0,6.0]])
    #final_goal = np.array([[5.0,5.0],[1.0,1.0], [5.0,1.0], [6.0,3.0], [3.0, 0.0]])

    #init_pos =   np.array([[1.0,3.0],[6.0,3.0], [3.0,1.0],[3.0,6.0], [4.5,1.5],[1.5,4.5], [4.5,4.5],[1.5,1.5]])
    #final_goal = np.array([[6.0,3.0],[1.0,3.0], [3.0,6.0],[3.0,1.0], [1.5,4.5],[4.5,1.5], [1.5,1.5],[4.5,4.5]])

    # give it a random seed if you are building on old array
    random.seed(time.time())
    mc_analysis(100, 15, loadFile='mcData/circle_ALL_10agents_100mcs.npy')

    mc_obst_analysis(50, 5, numAgents=2)
    mc_obst_analysis(50, 5, numAgents=3)
    ...
    # obstacleList = w.obstacleList
    # rrt = RRT(start=init_pos, goal=final_goal, obstacleList=obstacleList)
    # path_x, path_y, u_traj = rrt.Planning(animation=False, debugPlot=0)
    ...
    # np.save("mcTimes_8agent_25mcs", mcTimes)
