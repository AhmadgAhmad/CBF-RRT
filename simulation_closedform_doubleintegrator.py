...
class CBF_RRT:
    def __init__(self, initial_state, obstacle_list):
        ...
        self.T = 5.5  #Integration Length
        self.N = 100 # Number of Control Updates
        self.y0 = initial_state
        self.k_cbf = 0.5 #CBF coefficient
        self.k_cbf_2 = 1.5
    ...
    def QP_controller(self,x_current,u_ref):
        ...
        for i in range(0,len(self.x_obstacle)):
            h = (x1-self.x_obstacle[i][0])**2+(x3-self.x_obstacle[i][1])**2-self.x_obstacle[i][2]**2
            lfh = 2*(x1-self.x_obstacle[i][0])*x2+2*(x3-self.x_obstacle[i][1])*x4
            lffh = 2*x2**2+2*x4**2
            lgfh = 2*(x1-self.x_obstacle[i][0])*self.u1+2*(x3-self.x_obstacle[i][1])*self.u2
            self.m.addConstr((lffh+lgfh+self.k_cbf*h+self.k_cbf_2*lfh)>=0)
    ...
    def motion_planning(self,u_ref):
        x_current = self.y0
        x = np.zeros((4,0))  # pos: x[0], x[2], vel:x[1], x[3]
        u = np.zeros((2,0))
        ...
        for i in range(0,self.N):
            ...
            x_current[0] = x_current[0]+x_current[1]*delta_t+0.5*u_current[0]*delta_t**2
            x_current[1] = x_current[1]+delta_t*u_current[0]
            x_current[2] = x_current[2]+x_current[3]*delta_t+0.5*u_current[1]*delta_t**2
            x_current[3] = x_current[3]+delta_t*u_current[1]
...
if __name__ == "__main__":
    ...
    #Sphere = Obstacle_Sphere([obstacle_list[0][0],0,obstacle_list[0][1],0],obstacle_list[0][2])
